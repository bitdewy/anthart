...
var anthart = {
  ...
  scale: function(image) {
    ...
    var ctx = canvas.getContext('2d');
    anthart.drawImage(ctx, image.src, w, h);
    return canvas.toBuffer();
  },
  ...
  compositeIcon: function(stuff, smartPallet) {
    ...
    if (smartPallet) {
      var canvas = new Canvas(size, size);
      var ctx = canvas.getContext('2d');
      anthart.drawImage(ctx, stuff.icon, size, size);
      ...
      anthart.drawImage(ctx, stuff.base, size, size);
  ...
  drawImage: function(ctx, buffer, width, height) {
    var img = new Canvas.Image();
    img.src = buffer;
  ...
  home: function(wallpaper, items) {
    ...
    anthart.drawImage(ctx, buffer, width, height);
