...
var typedStream = function(canvas, type) {
  if (type === '.jpg') {
...
var dataFromExif = function(exif) {
  var map = {
    '1': {
      width: exif.image.ImageWidth,
      height: exif.image.ImageHeight,
      angle: 0
    },
    '6': {
      width: exif.image.ImageHeight,
      height: exif.image.ImageWidth,
      angle: 0.5 * Math.PI
    },
    '3': {
      width: exif.image.ImageWidth,
  ...
  return map[exif.image.Orientation];
};

var rotate = function(image) {
  if (image.exif) {
    var data = dataFromExif(image.exif);
    var w = data.width;
    var h = data.height;
    ...
    var imageWidth = image.exif.image.ImageWidth;
    var imageHeight = image.exif.image.ImageHeight;
...
var scalePrivate = function(image) {
  var w = image.width;
  var h = image.height;
  var canvas = new Canvas(w, h);
  var ctx = canvas.getContext('2d');
...
var anthart = {
  ...
  scale: function(image) {
    var canvas = scalePrivate(image);
    return canvas.toBuffer();
  },

  scaleToJpegStream: function(image) {
    var canvas = scalePrivate(image);
    return typedStream(canvas, '.jpg');
  },

  scaleToPngStream: function(image) {
    var canvas = scalePrivate(image);
    return typedStream(canvas, '.png');
  },
  ...
  crop: function(image) {
    var img = new Canvas.Image();
    img.src = image.src;
    var ratioW = img.width / image.canvas.width;
    var ratioH = img.height / image.canvas.height;
    var x = image.topLeft.x * ratioW;
    var y = image.topLeft.y * ratioH;
    var w = (image.bottomRight.x - image.topLeft.x) * ratioW;
    var h = (image.bottomRight.y - image.topLeft.y) * ratioH;
    var args = [img].concat([x, y, w, h]).concat([0, 0, w, h]);
    var canvas = new Canvas(w, h);
    var ctx = canvas.getContext('2d');
